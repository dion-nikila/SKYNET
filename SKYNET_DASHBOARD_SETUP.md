...
- Training dataset stage used by this project: `data/Airware-Haikou/2_filled_data/*.csv`
- This project does not directly train from `1_row_data` or directly consume `3_MTSAM`.
...
```bash
pip install optuna
python /Users/dionnikila/Desktop/SKYNET-IPD-01/model.py
```
...
- CO/pressure/wind mapping in SKYNET is inferred from observed `2_filled_data` ranges and validated against saved model metadata quantiles.
- The project uses `2_filled_data` (interpolated/filled stage), not direct `3_MTSAM` artifacts.
...
- For usability evidence collection, use:
